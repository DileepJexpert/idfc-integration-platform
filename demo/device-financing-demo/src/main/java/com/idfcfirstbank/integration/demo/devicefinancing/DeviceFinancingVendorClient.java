...
/**
 * REAL outbound HTTP to the device-financing vendor (the mock-vendors server in
 ...
 */
@Component
public class DeviceFinancingVendorClient implements DeviceFinancingVendor {

    private final DeviceFinancingDemoProperties props;
    private final RestClient http;

    public DeviceFinancingVendorClient(DeviceFinancingDemoProperties props) {
        this.props = props;
    ...
    public Map<String, Object> call(String operation, String brand, String deviceId, BrandRow row) {
        if (props.vendorBaseUrl() == null) {
            throw new CapabilityException(ErrorClass.PERMANENT,
                    "demo.device-financing.vendor-base-url not configured");
        }
    ...
    private String fetchToken(String scope) {
        if (props.tokenUrl() == null) {
            throw new CapabilityException(ErrorClass.PERMANENT,
                    "OAUTH brand but demo.device-financing.token-url not configured");
        }
        try {
