...
/**
 * The kyc {@link Capability} bean — the framework wires the Kafka shell AND the
 * idempotent dispatch around it, so a redelivered request (Kafka is
 * at-least-once) returns the first verification's result instead of calling
 * NSDL again.
 *
 * <p>The single "verify" operation delegates to the untouched {@link KycService}.
 */
@Component
public class KycCapability implements Capability {

    private final KycService service;

    public KycCapability(KycService service) {
    ...
    public List<CapabilityOperation> operations() {
        return List.of(delegating("verify"));
    }
