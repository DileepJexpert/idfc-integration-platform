...
/**
 * The bureau {@link Capability} bean — the framework wires the Kafka shell AND
 * the idempotent dispatch around it, so a redelivered request (Kafka is
 * at-least-once) returns the first pull's result instead of hitting the bureaus
 * again.
 *
 * <p>The single "pull" operation delegates to the untouched {@link BureauService}.
 */
@Component
public class BureauCapability implements Capability {

    private final BureauService service;

    public BureauCapability(BureauService service) {
    ...
    public List<CapabilityOperation> operations() {
        return List.of(delegating("pull"));
    }
